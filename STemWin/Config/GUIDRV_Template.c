...
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, int PixelIndex) {
    ...
    GUI_USE_PARA(PixelIndex);
    {
      ...
      //
			/* 使用驱动中的图形驱动 */
			NT35510_SetPointPixelColor(xPhys,yPhys,PixelIndex);
    }
...
static unsigned int _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) {
  unsigned int PixelIndex;
    ...
    GUI_USE_PARA(y);
    {
      ...
      //
			/* 使用驱动中的图形驱动 */
      PixelIndex = NT35510_GetPointPixel(xPhys,yPhys);
    }
    #if (LCD_MIRROR_X == 0) && (LCD_MIRROR_Y == 0) && (LCD_SWAP_XY == 0)
      #undef xPhys
      #undef yPhys
  ...
  if (GUI_pContext->DrawMode & LCD_DRAWMODE_XOR) {
    for (; y0 <= y1; y0++) {
      for (x = x0; x <= x1; x++) {
		...
		/* 使用驱动中的图形驱动 */
		NT35510_DrawRectangleColor(x0, y0, x1-x0+1, y1-y0+1, PixelIndex);
//    for (; y0 <= y1; y0++) {
//      for (x = x0; x <= x1; x++) {
